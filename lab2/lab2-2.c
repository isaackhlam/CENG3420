...
void handle_slli(unsigned int cur_inst) {
     ...
     */
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    NEXT_LATCHES.REGS[rd] = CURRENT_LATCHES.REGS[rs1] << MASK24_20(cur_inst);
}
...
void handle_srli(unsigned int cur_inst) {
     ...
     */
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    NEXT_LATCHES.REGS[rd] = (unsigned int)CURRENT_LATCHES.REGS[rs1] >> MASK24_20(cur_inst);
}
...
void handle_lui(unsigned int cur_inst) {
    ...
    NEXT_LATCHES.REGS[rd] = cur_inst & 0xFFFFF000;
...
void handle_lh(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst);
    NEXT_LATCHES.REGS[rd] = sext(read_mem(sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1]) & 0xFFFF, 16);
}
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[CURRENT_LATCHES.REGS[rs1]+imm_s+3] = MASK31_24(CURRENT_LATCHES.REGS[rs2]);
}

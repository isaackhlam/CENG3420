...
int inst_to_binary(
	...
	if (is_opcode(opcode) == ADDI) {
		...
		/* Lab2-1 assignment */
		binary = (0x04 << 2) + 0x03 + (1 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += ((validate_imm(arg3, 12, line_no) & 0b11111) << 20);
	} else if (is_opcode(opcode) == XORI) {
		/* Lab2-1 assignment */
		binary = (0x04 << 2) + 0x03 + (0b100 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);
		 ...
		 * tip: you may need the function `lower5bit`
		 */
		binary = (0x04 << 2) + 0x03 + (0b101 << 12) + (0b100000 << 25);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += ((validate_imm(arg3, 12, line_no) & 0b11111) << 20);
	} else if (is_opcode(opcode) == ORI) {
		/* Lab2-1 assignment */
		binary = (0x04 << 2) + 0x03 + (0b110 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);		
	} else if (is_opcode(opcode) == ANDI) {
		/* Lab2-1 assignment */
		binary = (0x04 << 2) + 0x03 + (0b111 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (MASK11_0(validate_imm(arg3, 12, line_no)) << 20);
		...
		binary = 0b110111;
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (handle_label_or_imm(arg2, label_table, cmd_no, line_no) & 0b11111111111111111111000000000000);
	}
	...
	// Integer Register-Register Operations
		...
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b100000 << 25);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == SLL) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (1 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == XOR) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b100 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == SRL) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b101 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == SRA) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b100 << 12) + (0b100000 << 25);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == OR) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b110 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	} else if (is_opcode(opcode) == AND) {
		/* Lab2-1 assignment */
		binary = 0b110011 + (0b111 << 12);
		binary += (reg_to_num(arg1, line_no) << 7);
		binary += (reg_to_num(arg2, line_no) << 15);
		binary += (reg_to_num(arg3, line_no) << 20);
	}
	...
	// Unconditional Jumps
		...
		// imm[19:12]
		binary += (offset & 0b11111111000000000000);
		// imm[11]
		binary += ((offset & 0b100000000000) << 9);
		// imm[10:1]
		binary += ((offset & 0b11111111110) << 20);
		// imm[20]
		binary += ((offset & 0b100000000000000000000) << 11);
	}
	...
	// Load and Store Instructions
		...
		int offset = ret->imm;
		// imm[4:0]
		binary += ((offset & 0b11111) << 7);
		// imm[11:5]
		binary += ((offset & 0b111111100000) << 20);
	} else if (is_opcode(opcode) == SH) {
		/* Lab2-1 assignment */
		binary = 0b100011 + (1 << 12);
		binary += (reg_to_num(arg1, line_no) << 20);
		struct_regs_indirect_addr* ret = parse_regs_indirect_addr(arg2, line_no);
		binary += (reg_to_num(ret->reg, line_no) << 15);
		int offset = ret->imm;
